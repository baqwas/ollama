...
PROMPT_1 = 'Which soccer stadium in the world has the largest seating capacity?'
response = ollama.chat(model=MODEL, messages=[
    {'role': 'user','content': PROMPT_1,},])
response_1 = response['message']['content']
print(f"{response_1}")
print(f"Total Duration: {(response['total_duration']/1e9):.2f} seconds")

PROMPT_2 = 'and for football?'
response = ollama.chat(model=MODEL, messages=[
    {'role': 'user','content': PROMPT_1,},
    {'role': 'assistant','content': response_1, },
    {'role': 'user','content': PROMPT_2,},])
response_2 = response['message']['content']
print(f"{response_2}")
print(f"Total Duration: {(response['total_duration']/1e9):.2f} seconds")

PROMPT_3 = 'and the largest seating capacity for any sport?'
response = ollama.chat(model=MODEL, messages=[
    {'role': 'user','content': PROMPT_1,},
    {'role': 'assistant','content': response_1, },
    {'role': 'user', 'content': PROMPT_2, },
...
response_3 = response['message']['content']
print(f"{response_3}")
print(f"Total Duration: {(response['total_duration']/1e9):.2f} seconds")
"""
                                        # performance matrix
...
print(f"Prompt evaluation duration: {(response.prompt_eval_duration/1e9):.2f}")
                                        # the time taken to load the model or components in nanoseconds
print(f"Load duration: {(response.load_duration/1e9):.2f}")
                                        # eval_count: The number of tokens evaluated during the generation. Here, 14 tokens.
print(f"Evaluation count: {response.eval_count}")
                                        # the time taken for the model to generate the response
print(f"Evaluation duration: {(response.eval_duration/1e9):.2f}")
print(f"Tokens applied: {len(response.context)}")
"""
